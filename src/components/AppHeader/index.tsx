...
const AppHeader: React.FC<IAppHeader> = (props) => {
  ...
  return (
    <div className="fixed w-screen flex flex-col items-center justify-center z-50">
      <TopBanner />
      <header className="md:mt-6 md:max-w-[calc(100vw-4rem)] lg:max-w-[calc(100vw-8rem)] md:rounded-full md:border md:p-4 md:pl-8 border-b backdrop-blur-xl relative bg-header flex justify-between items-center py-3 px-4 z-50 w-screen border-white/10">
        <Logo
          className="relative -top-1 md:-top-2 h-9 md:h-auto cursor-pointer"
          onClick={() => navigate('/')}
        />
        <div className="flex items-center md:gap-2 lg:gap-4">
          <Link to="/leaderboard" className="app-link py-2 md:px-2 lg:px-3 hidden md:flex">Leaderboard</Link>
